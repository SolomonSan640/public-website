...
class ContactPageController extends Controller
{
    ...
    public function index($country)
    {
        ...
        $contact = ContactPage::orderBy('updated_at', 'desc')->get();
    ...
    public function contactPage(Request $request)
    {
        ...
        if (strtolower($request->country) === 'mm') {
            $contact = ContactPageMM::orderBy('created_at')->get();
            if ($contact->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $contact[0]], 200);
        } else {
            $contact = ContactPage::orderBy('created_at')->get();
            if ($contact->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $contact[0]], 200);
    ...
    protected function getCreateData($request, $id)
    {
        ...
        $existingData = ContactPage::find($id);
