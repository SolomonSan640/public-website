...
class ProfilePageController extends Controller
{
    ...
    public function index($country)
    {
        ...
        $profile = ProfilePage::orderBy('updated_at', 'desc')->get();
    ...
    public function profilePage(Request $request)
    {
        ...
        if (strtolower($request->country) === 'mm') {
            $profile = ProfilePageMM::orderBy('created_at')->get();
            if ($profile->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, "message" => __('success.dataRetrieved'), 'data' => $profile[0]], 200);
        } else {
            $profile = ProfilePage::orderBy('created_at')->get();
            if ($profile->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, "message" => __('success.dataRetrieved'), 'data' => $profile[0]], 200);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $profile = ProfilePage::findOrFail($id);
    ...
    protected function getCreateData($request, $id)
    {
        ...
        $existingData = ProfilePage::find($id);
