...
class HomePageController extends Controller
{
    ...
    public function index($country)
    {
        ...
        $index = HomePage::orderBy('updated_at', 'desc')->get();
    ...
    public function homePage(Request $request)
    {
        ...
        if (strtolower($request->country) === 'mm') {
            $index = HomePageMM::orderBy('created_at')->get();
            if ($index->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $index[0]], 200);
        } else {
            $index = HomePage::orderBy('created_at')->get();
            if ($index->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $index[0]], 200);
    ...
    protected function getCreateData($request, $id)
    {
        ...
        $existingData = HomePage::find($id); // Assuming you are updating the record with ID 1
