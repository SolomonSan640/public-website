...
class ServicePageController extends Controller
{
    ...
    public function index($country)
    {
        ...
        $services = ServicePage::orderBy('updated_at', 'desc')->get();
    ...
    public function servicePage(Request $request)
    {
        ...
        if (strtolower($request->country) === 'mm') {
            $services = ServicePageMM::orderBy('created_at')->get();
            if ($services->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $services[0]], 200);
        } else {
            $services = ServicePage::orderBy('created_at')->get();
            if ($services->isEmpty()) {
                return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => null], 200);
            }
            return response()->json(['status' => 200, 'message' => __('success.dataRetrieved'), 'data' => $services[0]], 200);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $services = ServicePage::findOrFail($id);
    ...
    protected function getCreateData($request, $id)
    {
        ...
        $existingData = ServicePage::find($id);
